...
**0.4 Server foundation** — §3.2, §4 *(2026-07-15, `P0-land-0.4-auth-routes`: reconciled the three previously-unmerged route worktrees onto a fresh integration branch off `main` tip `fad6f3e`. Merge order: `P0-0.4-auth-challenge-route` (tip `5ca36a4`) first — conflicts in `CLAUDE.md` (package-layout table; combined the branch's `@falcon/server` auth-route note with `main`'s already-landed `@falcon/web` description) and `packages/server/src/config.ts` (pure formatting/line-wrap diff on the same `EnvSchema` fields; kept `main`'s style, content identical), plus `pnpm-lock.yaml` (regenerated via `pnpm install`). Then `P0-0.4-oauth-signin-routes` (tip `9eef49c`), built serially on top of `auth-challenge-route` — merged clean, no conflicts. Then `P0-0.4-pairing-endpoints` (tip `c954ac5`), branched independently from an earlier point (merge-base `03ff892` vs the other two's `c9823c4`) — one conflict in `packages/server/src/app/server.ts` (both sides added imports/route registrations to the same lines; resolved by keeping all three: `buildAuthRoutes`, `buildOAuthRoutes`, and `pairRoutes`, registered in that order — no path overlap: `/v1/auth`, `/v1/auth/register`, `/v1/auth/pair*`). `pnpm build && pnpm typecheck && pnpm test` all green on the integration branch before landing onto `main`.)*
