...
- [x] Exit semantics: Ctrl-C keeps session `active`/resumable; crash → `failed` via best-effort archive POST (PRD FR-3.7) **(landed 2026-07-16 via `P1-land-1.4-exit-semantics`:** merged `P1-1.4-exit-semantics` (tip `835843d`) `--no-ff` directly onto the shared `main` ref from the primary (non-worktree) checkout at `main`'s then-tip `78f22af` (merge-base `0bf99d4`, main's only intervening commit was a no-op "0 tasks completed" cycle marker — zero real drift, conflict-free merge). Adds `packages/server/src/app/routes/sessionStatus.ts` (idempotent one-way `POST /v1/sessions/:id/status` → `failed`, fans out `session-update`+`attention{kind:"failed"}` through the existing `EventRouterPort`, registered in `server.ts`), `packages/cli/src/api/sessionStatus.ts` (`reportSessionFailed`, best-effort/never-throws POST client), and `packages/cli/src/claude/sessionExit.ts` (`createSessionExitTracker` — classifies Ctrl-C/SIGTERM/SIGHUP as resumable `signal-exit`, no report; any other exit as `crash`, best-effort reported; never forwards signals to the child, preserving the real TUI's own Ctrl-C UX). Re-ran, forced (`--force`, no turbo cache) `pnpm build`/`pnpm typecheck`/`pnpm test` on the merged branch: all green — 5/5 build tasks, 8/8 typecheck tasks, 9/9 test tasks; `falcon` (cli) 220/220 tests (incl. the 13 new `api/sessionStatus.test.ts`/`claude/sessionExit.test.ts` cases), `@falcon/server` 145/145 (incl. the 5 new `sessionStatus.test.ts` cases), `@falcon/wire` 61/61 unaffected. See `task-summary/P1-land-1.4-exit-semantics.md` for the full landing report.)*
