...
### Cross-cutting (continuous, no phase) *(P0-land-cross-wire-schema-lint: merged `P0-cross-wire-schema-lint` (tip `6a31f5d`) into `main` — `packages/wire/scripts/check-additive-vs-base.ts` (CI-only lint re-deriving pre-change wire schemas from git history via `git archive` + re-running `isCompatible`/`describeShape`) wired into `.github/workflows/ci.yml`; `pnpm build`/typecheck/test all green post-merge. Closes the gap where a PR that breaks a schema AND regenerates the frozen fixture in the same commit previously passed `additiveOnly.test.ts` cleanly.)*
