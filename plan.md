...
**0.4 Server foundation** — §3.2, §4 *(2026-07-15: `P0-land-0.4-worktrees-onto-main` merged the current `main` tip into this branch, resolving the accumulated tracking-cycle drift (cycles 9–13, plus the intervening `P1-land-cli-scaffold` landing), then verified `pnpm build`/`pnpm typecheck`/`pnpm test`/`pnpm lint` all green on the merged tree — 50/55 `@falcon/server` tests passing, 5 `seq.test.ts` concurrency tests skipped (need a live Postgres), `@falcon/wire` 61/61, `@falcon/crypto` 65/65, 0 lint errors. This branch now actually contains the Drizzle schema, initial migration, `seq.ts`, and auth module in `packages/server/src/`, plus `docker-compose.dev.yml` at the repo root — landing this branch onto `main` (fast-forward or `--no-ff` merge) delivers all five bullets below in one step. `P0-0.4-auth-challenge-route` intentionally NOT included here: it only carries the same merged prerequisite commits, no new `POST /v1/auth` route code beyond that — landing its two route commits onto the new `main` tip remains a follow-up task)*
...
**1.1 Server realtime (read path)** — §4.1, §4.2
...
**1.2 Server write path (HTTP)** — §4.3 **(DELTA D1)**
...
**1.6 Web app v1 (read-only)** — §8.1–§8.4 **(DELTA D4)** *(cycle 9 note: `packages/web` Next.js App Router scaffold work exists complete in the unmerged `P1-1.6-web-app-scaffold` worktree, with task-summary doc; not merged into `main`, so this section stays unchecked and uncredited pending a merge + re-verification cycle. Cycle 10: re-confirmed still unmerged — `git merge-base --is-ancestor P1-1.6-web-app-scaffold main` fails. Cycle 12 note: this cycle's instructions asked to read `task-summary/P1-land-web-scaffold.md` as a "successful task" — that file does not exist on `main`; it exists only in `.worktrees/P1-land-web-scaffold/task-summary/` (branch built on `main`'s tip `2dcbde4`, merges `P1-1.6-web-app-scaffold` cleanly with no conflicts, self-reports `pnpm build`/`pnpm --filter @falcon/web typecheck` green and 158/158 tests across all 4 packages). `main` still has no `packages/web` directory, so still unchecked/uncredited — landing is out of this tracker's scope. Cycle 13 (2026-07-15): re-requested `task-summary/P1-land-web-scaffold.md` again — still does not exist on `main` (confirmed via `git merge-base --is-ancestor P1-land-web-scaffold main` → not an ancestor). A further land-attempt worktree has since appeared, also unmerged: `P1-land-web-scaffold-onto-main` (own `task-summary/P1-land-web-scaffold-onto-main.md`). `main`'s `packages/` still only has `cli`, `crypto`, `server`, `wire` — no change from cycle 12)*
