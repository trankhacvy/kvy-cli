...
- [x] Reducer port: multi-phase pipeline (perm placeholders, text, tool matching by name+args, tool results, sidechain linking, dedupe) + golden-trace test harness **(P** from happy-app `reducer.ts` — biggest single port**)** *(landed via `P1-land-1.6-reducer-port`, 2026-07-16 — merged `P1-1.6-reducer-port` (tip `71abb43`) onto `main` (base tip `171af64`); clean merge, no conflicts. `packages/web/src/sync/reducer/{reduce,types,index}.ts` + golden-trace test harness (`golden.test.ts` + `__testdata__/trace_*.json`) verified live on `main`: `pnpm build`/`pnpm typecheck`/`pnpm test` all green (55/55 `@falcon/web` tests, 87/87 server, 65/65 crypto, 61/61 wire, 168/168 cli).)*
