...
**1.4 Transcript pipeline** — §6.4, §6.5 *(cycle 16, 2026-07-15: landed via `P1-land-1.4-transcript-scanner` — `P1-1.4-transcript-scanner` merged cleanly onto `main`'s cycle-16 tip (`packages/cli/src/claude/` didn't exist on `main` before this merge; no overlap with `args.ts`/`home.ts`/`index.ts`/`logger.ts`). `pnpm build`/`typecheck`/`test` all green monorepo-wide (66/66 `falcon` tests incl. the new scanner/fileWatcher suites). Only the `sessionScanner`/`startFileWatcher` port bullets below are verified live on `main` — `mapClaudeToEnvelopes` and the HTTP outbox bullets are separate, still-open follow-up work.)*
