...
- [ ] Reducer port: multi-phase pipeline (perm placeholders, text, tool matching by name+args, tool results, sidechain linking, dedupe) + golden-trace test harness **(P** from happy-app `reducer.ts` — biggest single port**)** *(work is complete and verified in branch `P1-land-1.6-reducer-port` (merge commit `3aef5c1`, up to date with `main` tip `acd4126`, cycle 33): `packages/web/src/sync/reducer/{reduce,types,index}.ts` + golden-trace test harness (`golden.test.ts` + `__testdata__/trace_*.json`), `pnpm build`/`pnpm typecheck`/`pnpm test` all green (56/56 `@falcon/web` tests incl. 13 `reduce.test.ts` + 7 `golden.test.ts`, 140/140 server, 65/65 crypto, 61/61 wire, 168/168 cli). Not yet actually merged onto `main` — `main`'s `packages/web/src/sync/` still does not exist as of this note; checkbox stays unchecked until a real merge lands. See 1.6 header note above for the stale-branch history this task fixed.)*
