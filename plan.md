...
- [x] Crypto worker (`crypto-bridge`): keys in worker memory from IndexedDB; seal/open message API **(N)** *(landed via `P1-land-1.6-crypto-worker`, 2026-07-15 — merged onto `main`; `packages/web/src/crypto/{protocol,key-storage,worker-handler,worker,client,factory,index}.ts` verified live: `pnpm build`/`pnpm typecheck`/`pnpm test` all green, 36/36 `@falcon/web` tests)*
