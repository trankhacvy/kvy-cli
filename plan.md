...
- [x] Crypto worker (`crypto-bridge`): keys in worker memory from IndexedDB; seal/open message API **(N)** *(landed via `P1-land-1.6-crypto-worker-onto-main`, 2026-07-15 — merged onto `main` at tip `fad6f3e`; `packages/web/src/crypto/{protocol,key-storage,worker-handler,worker,client,factory,index}.ts` verified live on `main`: `pnpm build`/`pnpm typecheck`/`pnpm test` all green)*
