...
**0.4 Server foundation** — §3.2, §4 *(Fastify skeleton bullet verified on `main` 2026-07-15, cycle 7, re-verified cycle 8, re-verified cycle 9, re-verified cycle 10, re-verified cycle 11 — `pnpm typecheck`/`pnpm test` green, 18/18 `@falcon/server` tests; remaining bullets not started/not merged — `P0-0.4-drizzle-schema`, `P0-0.4-docker-compose-dev`, `P0-0.4-auth-module`, `P0-0.4-seq-allocator`, and `P0-0.4-auth-challenge-route` all exist complete in unmerged worktrees; cycle 11 read the `P0-0.4-seq-allocator`/`P0-0.4-auth-challenge-route` task-summaries and confirms both self-report full verification, but `main` itself still has none of this code — `packages/server` on `main` has no `src/db/` at all — so the checkboxes below stay unchecked until an actual merge lands; a ready fast-forwardable integration branch `P0-land-0.4-worktrees` (built directly on `main`'s tip, bundling drizzle-schema+docker-compose-dev+auth-module+seq-allocator, not yet including auth-challenge-route) now exists but is unlanded, not yet credited)*
