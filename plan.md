...
- [x] `claudeLocal.ts` port: spawn w/ `stdio ['inherit','inherit','inherit','pipe']`, **setBlocking(true) fix**, session-flag interception (`--resume/-r/--continue/-c/--session-id`), `--append-system-prompt`, hook `--settings` temp file, fd3 thinking reader **(P** — **Landed 2026-07-16 via `P1-land-1.3-claudelocal-spawn`:** merged `packages/cli/src/claude/claudeLocal.ts` + `claudeLocal.test.ts` directly onto `main` (merge-base `2c721e9` behind `main`'s then-tip `a7bbceb`; `git merge --no-ff` conflict-free, new files only). Re-verified on `main` post-merge: forced `pnpm exec turbo run build` 5/5, `pnpm exec turbo run typecheck` 7/7, `pnpm exec turbo run test` 9/9 (`falcon` cli 206/206, `@falcon/server` 140/140). Sanity-checked the task-summary's flagged behavioral judgment call by reading `resolveSessionFlags`/`extractFlag` directly: a bare trailing `--resume`/`-r` (no value, or followed by another flag) makes `extractFlag(..., withValue: true)` return `found: false`, leaving the flag untouched in `claudeArgs` so it passes straight through to Claude Code's own interactive picker — matches falcon-plan.md's stated goal ("`falcon --resume` behaves exactly like `claude --resume`"), confirmed as intended, no change needed.**)
