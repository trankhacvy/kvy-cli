...
- [ ] Crypto worker (`crypto-bridge`): keys in worker memory from IndexedDB; seal/open message API **(N)** *(cycle 16, 2026-07-15: `task-summary/P1-1.6-crypto-worker.md` requested for this cycle does not exist on `main` — confirmed via `git merge-base --is-ancestor`. The worker (`packages/web/src/crypto/{protocol,key-storage,worker-handler,worker}.ts`) exists complete and self-verified only in worktree `.worktrees/P1-1.6-crypto-worker` (its own task-summary reports 26/26 `@falcon/web` tests, 9/9 monorepo tasks green). `main`'s `packages/web/src/crypto/` does not exist yet — checkbox stays unchecked pending merge. **Re-confirmed cycle 17, 2026-07-15:** `task-summary/P1-land-1.6-crypto-worker.md` also requested this cycle — same result, does not exist on `main` (`git merge-base --is-ancestor P1-land-1.6-crypto-worker main` → not an ancestor). A land-branch (`P1-land-1.6-crypto-worker`, commits through `1be84b9`, includes test-failure and code-review fixes) now exists and self-reports green, but was never fast-forwarded/merged onto `main` — `packages/web/src/crypto/` still absent from `main`. Checkbox remains unchecked. **Re-confirmed cycle 19, 2026-07-15:** `task-summary/P1-land-1.6-crypto-worker-onto-main.md` requested this cycle — same result, does not exist on `main` (`git merge-base --is-ancestor P1-land-1.6-crypto-worker-onto-main main` → not an ancestor). A further land-branch (`P1-land-1.6-crypto-worker-onto-main`, tip `7b38ccf`, 9 commits ahead, merge-base exactly `main`'s current tip `fad6f3e` — zero drift) self-reports green but likewise was never fast-forwarded/merged onto `main`. Checkbox remains unchecked.)*
...
### Cross-cutting (continuous, no phase)
