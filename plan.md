...
### Cross-cutting (continuous, no phase) *(Cycle 24, 2026-07-16: `task-summary/P0-cross-wire-schema-lint.md` requested this cycle does not exist on `main` — confirmed via directory listing and `git merge-base --is-ancestor P0-cross-wire-schema-lint main` → not an ancestor. Real, complete, self-verified work sits unmerged in worktree `.worktrees/P0-cross-wire-schema-lint` (tip `6a31f5d`, feat+fix+refactor commits): `packages/wire/scripts/check-additive-vs-base.ts`, a CI-only lint that re-derives the pre-change wire schemas from git history (base ref resolution + `git archive` extraction) and re-runs the existing `isCompatible`/`describeShape` logic against them — closing the gap where a PR that breaks a schema AND regenerates the frozen fixture in the same commit currently passes `additiveOnly.test.ts` cleanly. Its own task-summary verified the hole is real (deleted a field, regenerated fixture, `additiveOnly.test.ts` still 38/38 green) before building the fix. The bullet below stays unchecked until an actual merge lands; landing is out of this tracker's scope. **Cycle 25, 2026-07-16:** `task-summary/P0-land-cross-wire-schema-lint.md` requested this cycle — does not exist on `main`'s `task-summary/` directory, and `git merge-base --is-ancestor P0-land-cross-wire-schema-lint main` → **not an ancestor**. This land-branch (tip `003a75c`) `--no-ff` merges `P0-cross-wire-schema-lint` into a fresh worktree off `main` and, per its own task-summary, also found and fixed a real gap during landing verification (`packages/wire`'s `lint:additive` script depends on `tsx`, which was only resolving via a global npm install on the task's machine, not a workspace devDependency — fixed by adding `"tsx": "^4.20.0"` to `packages/wire/package.json`, which a clean CI runner would otherwise fail). Its own task-summary reports `pnpm build`/`typecheck`/`test` all green (9/9 tasks, `additiveOnly.test.ts` 38/38). Despite the task-summary's own narrative saying it "Landed the ... branch onto `main`," the merge only ever touched the new branch `P0-land-cross-wire-schema-lint` in its own worktree — the shared `main` ref is unchanged (`packages/wire/scripts/check-additive-vs-base.ts` and the `tsx` devDependency are both absent from `main`). The bullet below stays unchecked; landing is out of this tracker's scope.)*
- [ ] Wire-schema additive-only lint in CI (runs from Phase 0 onward)
...
- [ ] MIT attribution headers on every ported Happy file
