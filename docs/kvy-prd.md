...
Time-to-magic-moment target: **< 5 minutes** from `npm install -g kvy` (or curl installer) to controlling a session from a second device.
...
- **[P0] FR-1.1** Single-command install: `npm install -g kvy` **and** `curl -fsSL https://kvy.dev/install.sh | sh` (standalone binary; no Node required for the curl path). Support macOS (arm64/x64) and Linux (x64) at MVP; Windows `[P2]`.
