...
- [x] Regenerate the fixture: pnpm --filter @falcon/wire exec tsx scripts/snapshot-shapes.ts (never hand-edit wire-shapes.json). Note: additiveOnly.test.ts only WARNS about unfrozen schemas, so this step must not be skipped on the grounds that tests pass.
...
- **Phase 5 (toolbar UI + docs):** `GitToolbar.tsx` (inline rename, commit, push, force-push
  ...
  falcon-prd.md, plan.md, and CLAUDE.md updated to reflect shipped state (including correcting a
