...
**Acceptance:** `pnpm --filter @kvy/wire build && pnpm --filter @kvy/wire test` passes, including additiveOnly.test.ts with the four new fixture entries; `GithubChecksParams`, `GithubChecksResult`, `CheckRun`, `PullRequestInfo` are importable from `@kvy/wire` (verify via the typecheck task).

### Phase 2: Phase 2 — CLI GitHub auth: machine-local token store + `kvy github login|logout|status`

- [x] Create packages/cli/src/github/githubAuth.ts — port of packages/cli/src/auth/credentials.ts's exact pattern: zod-validated JSON `{ token: string, createdAt: number, scope?: string, method: "device-flow" | "pat" }` at `~/.kvy/github.key`, written with mode 0o600 (same sync writeFileSync+chmodSync shape as `auth/credentials.ts`'s own precedent — see that module's own doc comment for why this isn't the separate async tmp-write+rename `persistence.ts` path), `readGithubToken(homeDir?)` never throws (returns null on missing/corrupt), `writeGithubToken(...)`, `clearGithubToken(...)`, injectable homeDir via resolveHomeDir default. The token value must never be passed to the logger.
...
- [x] Create packages/cli/src/commands/github.ts implementing: `kvy github login [--token] [--client-id <id>]` — with `--token`, prompt for a PAT on stdin (never accept the token as an argv value); otherwise run the device flow, printing the user code + verification URI, with client id resolved `--client-id` flag → `KVY_GITHUB_CLIENT_ID` env → a `DEFAULT_GITHUB_CLIENT_ID` constant (empty string until a Kvy OAuth app exists — when empty and no override, print an instruction to use `--token` or set the env var and exit non-zero); on success write via githubAuth.ts and print the granted scope. `kvy github logout` — clearGithubToken. `kvy github status` — token present? then GET https://api.github.com/user (Authorization: Bearer, X-GitHub-Api-Version: 2022-11-28) and report login + scopes (from the X-OAuth-Scopes response header) or 'invalid token'.
- [x] Wire the command into packages/cli/src/args.ts (new `github` command type with `login|logout|status` actions + `--token`/`--client-id` flags, following parseWorkspaceConfig's flag-parsing shape at ~line 235-257) and packages/cli/src/index.ts dispatch, following exactly how `workspace`/`adapters` commands are registered. No daemon interaction — this is a local file + HTTPS operation, same precedent as `kvy workspace config`.
...
**Acceptance:** `pnpm --filter kvy test` passes with the new github/ and commands/github tests; manual smoke: `kvy github login --token` (pasting a PAT) creates `~/.kvy/github.key` with mode 0600, `kvy github status` prints the GitHub login, `kvy github logout` removes it; grep confirms the token string is never interpolated into any logger call.
...
**Acceptance:** `pnpm --filter @kvy/web build && pnpm --filter @kvy/web test` green; the ChecksPanel render test proves all six UI states (5 wire states + daemon-unsupported) produce their distinct copy; `next build` static export emits the /session/[id]/checks route without errors.
...
- `pnpm build && pnpm typecheck && pnpm test` run from a clean `--force` (no
  ...
  exactly — `@kvy/wire` 111, `kvy` cli 1618, `@kvy/web` 818,
  `@kvy/server` 316, `@kvy/e2e` 1.
...
- Smoke-tested the CLI for real against a scratch `KVY_HOME_DIR` (no
  real GitHub account touched): `kvy github login --token` (piped a
  fake PAT via stdin) created `~/.kvy/github.key` at mode `0600`
  (verified via `stat`); `kvy github status` made one real, read-only
  `GET https://api.github.com/user` call and correctly reported "invalid
  token — GitHub rejected it"; `kvy github logout` removed the file;
  `kvy github login` with no client id configured fails fast with the
...
flag genuinely rejects a bare pasted value (`kvy github login --token
gho_x` throws `Unknown "kvy github login" flag`, confirmed by both
...
`KVY_HOME_DIR`-aware default when the daemon calls them with no
explicit override).
...
test` all green post-merge (145 test files, 1660 tests in the `kvy` cli
