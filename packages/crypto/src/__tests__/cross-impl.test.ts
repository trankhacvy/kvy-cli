...
describe('cross-impl: NaCl box (libsodiumEncryptForPublicKey / libsodiumDecryptWithSecretKey)', () => {
  it('same seed derives the same public key on both platforms', () => {
    ...
    expect(node.libsodiumPublicKeyFromSecretKey(seed)).toEqual(web.libsodiumPublicKeyFromSecretKey(seed));
  });

  it('node encrypts -> web decrypts, and vice versa', () => {
    const recipient = deriveKeyTree(node.getRandomBytes(32)).content;

    const fromNode = node.libsodiumEncryptForPublicKey(new TextEncoder().encode('from node'), recipient.publicKey);
    expect(web.libsodiumDecryptWithSecretKey(fromNode, recipient.secretKey)).toEqual(new TextEncoder().encode('from node'));

    const fromWeb = web.libsodiumEncryptForPublicKey(new TextEncoder().encode('from web'), recipient.publicKey);
    expect(node.libsodiumDecryptWithSecretKey(fromWeb, recipient.secretKey)).toEqual(new TextEncoder().encode('from web'));
  });
});
