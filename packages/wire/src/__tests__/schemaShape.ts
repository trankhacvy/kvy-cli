...
export function describeShape(schema: z.ZodTypeAny): ShapeDescriptor {
  ...
  switch (type) {
    ...
    case 'optional':
      return { kind: 'optional', inner: describeShape((schema as unknown as { unwrap(): z.ZodTypeAny }).unwrap()) };
    case 'nullable':
      return { kind: 'nullable', inner: describeShape((schema as unknown as { unwrap(): z.ZodTypeAny }).unwrap()) };
    case 'array':
      return { kind: 'array', element: describeShape((schema as unknown as { element: z.ZodTypeAny }).element) };
    case 'record': {
...
export function isCompatible(prev: ShapeDescriptor, next: ShapeDescriptor): boolean {
  ...
  switch (prev.kind) {
    ...
    case 'union': {
      ...
      return prev.options.every((prevOpt) => n.options.some((nextOpt) => isCompatible(prevOpt, nextOpt)));
    }
    case 'optional':
    case 'nullable': {
