...
describe("EphemeralSchema", () => {
  it("accepts activity/machine-presence/attention", () => {
    expect(
      EphemeralSchema.safeParse({ t: "activity", sessionId: "s1", working: true }).success,
    ).toBe(true);
    expect(
      EphemeralSchema.safeParse({ t: "machine-presence", machineId: "m1", online: false }).success,
    ).toBe(true);
    expect(
      EphemeralSchema.safeParse({ t: "attention", sessionId: "s1", kind: "perm" }).success,
    ).toBe(true);
  });

  it("rejects an unknown attention kind", () => {
    expect(
      EphemeralSchema.safeParse({ t: "attention", sessionId: "s1", kind: "urgent" }).success,
    ).toBe(false);
  });
});
