...
import { createEnvelope, SessionEnvelopeSchema, SessionEventSchema, type SessionEvent } from './session';
...
describe('SessionEventSchema', () => {
  it('accepts all 12 event variants', () => {
    const events: SessionEvent[] = [
      ...
      { t: 'file', ref: 'upload-2', name: 'image.png', size: 2048, image: { w: 100, h: 80, thumbhash: 'abc' } },
      { t: 'turn-start' },
    ...
    for (const event of events) {
      ...
      expect(result.success, `expected ${event.t} to parse: ${JSON.stringify(result.success ? null : result.error)}`).toBe(
        true
      );
    }
  });
