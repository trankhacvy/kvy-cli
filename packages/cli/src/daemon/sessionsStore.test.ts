...
describe("sessionsStore", () => {
  ...
  it("round-trips the optional pid field (boot-time re-adoption, readoptSessions.ts)", async () => {
    const session = fixture({ pid: 51245 });
    await persistSession(homeDir, session);

    expect((await readPersistedSessions(homeDir)).sess_1?.pid).toBe(51245);
  });

  it("drops a record whose `pid` is present but not a number", async () => {
    await writeFile(
      sessionsFilePath(homeDir),
      JSON.stringify({
        schemaVersion: 3,
        sessions: {
          sess_bad: { ...fixture({ sessionId: "sess_bad" }), pid: "not-a-number" },
          sess_good: fixture({ sessionId: "sess_good", pid: 4242 }),
        },
      }),
      "utf8",
    );
  ...
  it("loads a pre-v3 schemaVersion:2 record with no `pid` key as `pid === undefined` (backward compat)", async () => {
    await writeFile(
      sessionsFilePath(homeDir),
      JSON.stringify({
        schemaVersion: 2,
    ...
    expect(legacy?.pid).toBeUndefined();
