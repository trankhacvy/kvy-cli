...
import { startHookServer, writeHookSettingsFile } from "./hookServer.js";
...
describe("writeHookSettingsFile", () => {
  ...
  it("writes a settings file whose SessionStart hook command embeds the given port", () => {
    ...
    try {
      ...
      const forwarderPath = hook.command.match(/^node "(.+\.cjs)" \d+$/)?.[1];
  ...
  it("end-to-end: the generated forwarder script actually delivers stdin JSON to the hook server", async () => {
    ...
    try {
      ...
      const forwarderPath = command.match(/^node "(.+\.cjs)" (\d+)$/)?.[1] as string;

      const child = spawn(process.execPath, [forwarderPath, String(server.port)], {
