...
function makeOptions(
  overrides: Partial<{ signal: AbortSignal; toolUseID: string; agentID: string }> = {},
) {
  return {
...
function createHandler(
  overrides: Partial<ConstructorParameters<typeof PermissionHandler>[0]> = {},
) {
...
/**
 * Extracts the reqId from the most recently emitted envelope. Throws a clear
 * error (rather than an unsafe optional-chain TypeError) if no envelope has
 * been emitted yet.
 */
function lastReqId(envelopes: SessionEnvelope[]): string {
  const ev = envelopes.at(-1)?.ev;
  if (!ev) {
    throw new Error("lastReqId: no envelope has been emitted yet");
  }
...
describe("PermissionHandler auto-rules", () => {
  ...
  it("allows edit tools in acceptEdits mode but still prompts for Bash", async () => {
    ...
    const editResult = await handler.canUseTool(
      "Write",
...
describe("PermissionHandler allow-for-session", () => {
  ...
  it("allow scope:session on Bash allow-lists only the exact literal command", async () => {
    ...
    const secondReqId = lastReqId(envelopes);
...
describe("PermissionHandler reset", () => {
  it("rejects every pending request, clears allow-lists and mode, and emits a perm-resolve for each", async () => {
    ...
    const pendingA = handler.canUseTool(
      "Read",
      { file_path: "/a" },
      makeOptions({ toolUseID: "a" }),
    );
    const pendingB = handler.canUseTool(
      "AskUserQuestion",
      { question: "?" },
      makeOptions({ toolUseID: "b" }),
    );
