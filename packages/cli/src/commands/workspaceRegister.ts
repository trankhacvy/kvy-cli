 ...
 * matching Omnara's low-friction UX per the PRD — `falcon workspace
 * register` with no flags just registers "here". No daemon interaction:
 * this reads/writes `~/.falcon/workspaces.json` directly, same rationale as
...
export async function runWorkspaceRegisterCommand(
  ...
  write(`falcon workspace register: registered ${describeEntry(entry)}\n`);
  return 0;
}

/** Runs `falcon workspace list`. */
export async function runWorkspaceListCommand(deps: WorkspaceRegistryCommandDeps): Promise<number> {
  ...
  if (entries.length === 0) {
    write("falcon workspace list: no registered workspaces\n");
    return 0;
  }
...
/** Runs `falcon workspace unregister`. Returns 1 if `directory` wasn't registered, so scripting can detect a no-op — never throws. */
export async function runWorkspaceUnregisterCommand(
  ...
  if (removed) {
    write(`falcon workspace unregister: removed ${directory}\n`);
    return 0;
  }
  write(`falcon workspace unregister: ${directory} was not registered\n`);
  return 1;
}
