/**
 * Per-line shiki syntax highlighting for the unified diff viewer. Returns
 * `ThemedToken[][]` (one array per line) rather than pre-rendering to HTML -
 * callers turn each token into a plain `<span>`, avoiding `dangerouslySetInnerHTML`.
 */
