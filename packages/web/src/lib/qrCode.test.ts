...
describe("encodeQrMatrix", () => {
  it("returns a square grid matching the reported size", () => {
    const { size, modules } = encodeQrMatrix("https://falcon.example/session/abc123/");
  ...
  it("produces a different (non-trivial) matrix for different input text", () => {
    const a = encodeQrMatrix("https://falcon.example/session/aaaaaaaa/");
    const b = encodeQrMatrix("https://falcon.example/session/bbbbbbbb/");
    expect(a.modules).not.toEqual(b.modules);
  });

  it("is deterministic for the same input", () => {
    const first = encodeQrMatrix("https://falcon.example/session/abc123/");
    const second = encodeQrMatrix("https://falcon.example/session/abc123/");
    expect(first).toEqual(second);
  });

  it("grows to fit a longer payload", () => {
    const short = encodeQrMatrix("https://f.example/s/1/");
    const long = encodeQrMatrix(`https://falcon.example/session/${"a".repeat(200)}/`);
...
describe("qrMatrixToPath", () => {
  ...
  it("matches the module count of a real encoded matrix", () => {
    const matrix = encodeQrMatrix("https://falcon.example/session/abc123/");
