        ...
        <div className="flex flex-col gap-2 pl-6">
          ...
          <div className="flex flex-col gap-1.5 text-sm font-medium">
            ...
            <BaseBranchPicker
              ...
              onSelect={(name) => onChange({ baseBranch: name })}
 ...
 * #16 "Searchable base-branch picker") for `"new-branch"` mode — a new
 * branch always used to fork off whatever's currently checked out at
 * `directory` with no way to pick a different base; this surfaces that base
 * as a real, searchable choice over the repo's actual local branches (e.g.
 * `main`/`master`), reusing the exact same `listBranches` RPC/data source as
 * `ExistingBranchPicker` below. Lazily fetches the branch list the first
 * time the trigger is opened (rather than eagerly on mount, like
 * `ExistingBranchPicker` does) — picking a base branch is optional, so a
 * session spawned without ever opening this picker shouldn't pay for a
 * `git.branches` round-trip it doesn't need. `selected === ""` means "no
 * base picked" — `wizard-state.ts`'s `branchOptionFromForm` then omits
 * `from` entirely, falling back to the pre-existing "branch off HEAD"
 * behavior.
 */
