...
/**
 * Dispatches on lifecycle events only (`perm`, `question`, `done`/turn-completed,
 * `failed`) — generic per-message pushes were deliberately ruled out (one buzz
 ...
 * `question`/`done`).
 *
 * Presence suppression here also requires the session's room to be joined
 * (`PresencePort.hasActiveVisibleClient`), not just "any non-machine client
 * connected" — see `eventRouter.ts`'s implementation and its comment on how
 * that room-scoping degrades gracefully today.
 *
 * `pushSubscriptions.channel` fans out to a pluggable `channels` registry
 * (`webpush` | `telegram` | `ntfy`) instead of a single hardcoded sender —
 * iOS Web Push is unreliable and the notification itself is the product, so a
 * single point of failure there is not acceptable.
 *
 * Quiet controls (per-session mute + mute-all): checked right alongside
 * presence, before any channel fan-out. An explicit user setting, so it
 * applies uniformly to every channel and every follow-up re-notify attempt —
 * there's no "mute Telegram but not ntfy" concept, and a muted session that
 * later gets un-muted picks the very next lifecycle event back up
 ...
 * this fire-and-forget: `void dispatchSessionEventPush(...)`.
 *
 * Re-notify scheduling: a `perm`/`question` dispatch is a request awaiting a
  ...
  async function attempt(params: {
    ...
    try {
      try {
        ...
        // Fail OPEN on a broken presence check — a presence bug must never
