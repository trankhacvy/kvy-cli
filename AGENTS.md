...
for the "why", read `docs/kvy-system-design.md` (architecture/protocol design) and
`docs/kvy-prd.md` (product requirements).

This is the shared entry point for any coding agent working in this repo (Claude
Code, Codex, or otherwise) — see `CLAUDE.md` for the Claude Code pointer to this
file.
...
`--model haiku` selects the model (`extractModelFlag`, `commands/start.ts`). `kvy
codex [args…]` runs the same wrapper against Codex instead.
...
Drive the CLI from a terminal (tmux is convenient for keeping it running in the
background — keep the model on **haiku** to stay cheap) and drive the web app
with whatever browser-automation tooling your agent has available (this repo's
own testing has used Claude Code's Chrome MCP integration — load its tools with
one `ToolSearch` call:
`select:mcp__claude-in-chrome__tabs_context_mcp,…navigate,…computer,…read_page,…tabs_create_mcp,…form_input,…read_console_messages`
— substitute your own equivalent if you're a different agent).
...
2. **Prepare the account (browser):** open `http://localhost:3000/signin/` (`/` is the public
...
3. **Pair the CLI (tmux):** with the env vars above, run `kvy auth login`. It prints a pairing
   URL/QR — open that URL in the already-signed-in browser tab and **approve**. The CLI now holds a
...
5. **Verify (browser):** the session appears on Home; open its timeline. Exercise the auth
   ...
   attempts.
6. **Key sharing (browser):** open a second browser profile, sign in with the same
...
- `docs/kvy-system-design.md` — architecture, protocol, and encryption design.
- `docs/kvy-prd.md` — product requirements.
- `docs/packages-guide.md` — per-package internals, deeper than the layout table above.
...
- `deploy/README.md` — self-host and production deployment walkthrough
  (`deploy/docker-compose.yml`: server + postgres + optional minio, migrate-on-boot,
  split-origin web with strict CSP + SRI; plus the Vercel/external-Postgres/R2 path).
- `CONTRIBUTING.md` — branch model, CI, and release mechanics.
- `docs/.archive/known-issues.md` — a running internal bug tracker, intentionally
  **not tracked in git** (see `.gitignore`'s `docs/.archive/`). Read it directly off
  disk when present; don't recreate it from git history.
- `docs/.archive/git-workflow.md` — simple internal reference for the branch →
  changeset → merge-to-release flow, same untracked convention as above.
