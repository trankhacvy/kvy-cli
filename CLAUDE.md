...
└─ web/       @falcon/web     Next.js PWA (App Router, static export). Tailwind + shadcn/ui
                              ...
                              worker bridge (src/crypto/) and apiSocket, the user-scoped
                              Socket.IO client with infinite reconnect + app-state
                              reporting (src/sync/), are wired up. Auth pages, the sync
                              engine (gap detection, TanStack Query invalidation), and the
                              reducer still [planned].
```
